...
class Datasiswa extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $tpAktif  = $tp[array_search('1', array_column($tp, 'active'))];
        $smtAktif = $smt[array_search('1', array_column($smt, 'active'))];
        ...
        $data = [
            ...
            'kelass'     => $this->dropdown->getAllKelas($tpAktif->id_tp, $smtAktif->id_smt),
        ];
    ...
    public function list()
    {
        ...
        $count  = $this->master->getSiswaTotalPage($tp->id_tp, $smt->id_smt, $filter, $search);
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data = ['insert' => false, 'text' => 'Data sudah ada. Pastikan NIS, NISN, dan Username belum digunakan siswa lain.'];
        } else {
            $insert = [
            ...
            $this->db->insert('buku_induk', ['id_siswa' => $id, 'uid' => $siswa->uid, 'status' => 1]);
            $data = ['insert' => true, 'text' => 'Siswa berhasil ditambahkan'];
        }
    ...
    public function edit($id)
    {
        ...
        $inputData = [
            ['label' => 'Nama Lengkap',    'name' => 'nama',          'value' => $siswa->nama,          'icon' => 'far fa-user',         'class' => '', 'type' => 'text'],
            ['label' => 'NIS',             'name' => 'nis',           'value' => $siswa->nis,           'icon' => 'far fa-id-card',      'class' => '', 'type' => 'number'],
            ['label' => 'NISN',            'name' => 'nisn',          'value' => $siswa->nisn,          'icon' => 'far fa-id-card',      'class' => '', 'type' => 'text'],
            ['label' => 'Jenis Kelamin',   'name' => 'jenis_kelamin', 'value' => $siswa->jenis_kelamin, 'icon' => 'fas fa-venus-mars',   'class' => '', 'type' => 'text'],
            ['label' => 'Diterima di kelas', 'name' => 'kelas_awal',    'value' => $siswa->kelas_awal,    'icon' => 'fas fa-graduation-cap', 'class' => '', 'type' => 'text'],
            ['label' => 'Tgl diterima',    'name' => 'tahun_masuk',   'value' => $siswa->tahun_masuk,   'icon' => 'far fa-calendar-alt', 'class' => 'tahun', 'type' => 'text'],
            ['label' => 'Sekolah Asal',    'name' => 'sekolah_asal',  'value' => $siswa->sekolah_asal,  'icon' => 'fas fa-graduation-cap', 'class' => '', 'type' => 'text'],
            ['label' => 'Status',          'name' => 'status',        'value' => $siswa->status,        'icon' => 'far fa-user',         'class' => 'status', 'type' => 'text'],
        ];

        $inputBio = [
            ['name' => 'tempat_lahir',  'label' => 'Tempat Lahir',     'value' => $siswa->tempat_lahir,  'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'tanggal_lahir', 'label' => 'Tanggal Lahir',    'value' => $siswa->tanggal_lahir, 'icon' => 'far fa-calendar', 'class' => 'tahun', 'type' => 'text'],
            ['name' => 'agama',         'label' => 'Agama',            'value' => $siswa->agama,         'icon' => 'far fa-star',     'class' => '', 'type' => 'text'],
            ['name' => 'alamat',        'label' => 'Alamat',           'value' => $siswa->alamat,        'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'rt',            'label' => 'RT',               'value' => $siswa->rt,            'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'rw',            'label' => 'RW',               'value' => $siswa->rw,            'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'kelurahan',     'label' => 'Kelurahan/Desa',   'value' => $siswa->kelurahan,     'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'kecamatan',     'label' => 'Kecamatan',        'value' => $siswa->kecamatan,     'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'kabupaten',     'label' => 'Kabupaten/Kota',   'value' => $siswa->kabupaten,     'icon' => 'far fa-map',      'class' => '', 'type' => 'text'],
            ['name' => 'kode_pos',      'label' => 'Kode Pos',         'value' => $siswa->kode_pos,      'icon' => 'far fa-envelope', 'class' => '', 'type' => 'text'],
            ['name' => 'hp',            'label' => 'HP',               'value' => $siswa->hp,            'icon' => 'fas fa-phone',    'class' => '', 'type' => 'text'],
        ];
    ...
    public function updateData()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data = ['insert' => false, 'text' => 'NIS kurang dari 6 angka, atau data sudah ada. Pastikan NIS dan NISN belum digunakan siswa lain.'];
        } else {
            ...
            $data = ['insert' => $input, 'text' => 'Siswa berhasil diperbaharui'];
        }
    ...
    public function deleteFile($id_siswa)
    {
    ...
    public function delete()
    {
        ...
        $this->output_json(['status' => true, 'total' => count($chk), 'last' => $aksi]);
    }
    ...
    public function do_import()
    {
        ...
        if (count($errors) > 0) {
            $this->output_json(['status' => false, 'errors' => $errors, 'duplikat' => $duplikat]);
            return;
        }
        ...
        foreach ($input as $value) {
            $siswa = [
                ...
                'kelas_awal' => $value['8'] ?? '',
                'tahun_masuk' => $value['9'] ?? '',
                'sekolah_asal' => $value['10'] ?? '',
                'tempat_lahir' => $value['11'] ?? '',
                'tanggal_lahir' => $value['12'] ?? '',
                'agama' => $value['13'] ?? '',
                'hp' => $value['14'] ?? '0',
                'email' => $value['15'] ?? '',
                'anak_ke' => $value['16'] ?? '1',
                'status_keluarga' => $value['17'] ?? '1',
                'alamat' => $value['18'] ?? '',
                'rt' => $value['19'] ?? '',
                'rw' => $value['20'] ?? '',
                'kelurahan' => $value['21'] ?? '',
                ...
                'kode_pos' => $value['25'] ?? '',
                'nama_ayah' => $value['26'] ?? '',
                'tgl_lahir_ayah' => $value['27'] ?? '',
                'pendidikan_ayah' => $value['28'] ?? '',
                'pekerjaan_ayah' => $value['29'] ?? '',
                'nohp_ayah' => $value['30'] ?? '',
                'alamat_ayah' => $value['31'] ?? '',
                'nama_ibu' => $value['32'] ?? '',
                'tgl_lahir_ibu' => $value['33'] ?? '',
                'pendidikan_ibu' => $value['34'] ?? '',
                'pekerjaan_ibu' => $value['35'] ?? '',
                'nohp_ibu' => $value['36'] ?? '',
                'alamat_ibu' => $value['37'] ?? '',
                'nama_wali' => $value['38'] ?? '',
                'tgl_lahir_wali' => $value['39'] ?? '',
                'pendidikan_wali' => $value['40'] ?? '',
                'pekerjaan_wali' => $value['41'] ?? '',
                'nohp_wali' => $value['42'] ?? '',
                'alamat_wali' => $value['43'] ?? '',
                'foto' => 'uploads/foto_siswa/' . ($value['3'] ?? '') . '.jpg',
            ];
            $this->db->set('uid', 'UUID()', FALSE);
            $this->db->insert('master_siswa', $siswa);
        }
        ...
        foreach ($uids as $uid) {
            ...
            if ($exists == 0) {
                $this->db->insert('buku_induk', (array) $uid);
            }
        }
        $this->db->trans_complete();

        $this->output_json(['status' => true, 'errors' => []]);
    }
    ...
    public function updateAll()
    {
        ...
        foreach ($input as $value) {
            ...
            $save = $this->db->update('master_siswa', $siswa, ['id_siswa' => $value['44']]);
        }
        $this->db->trans_complete();
        $this->output_json(['status' => $save ?? false, 'errors' => []]);
    }
    ...
    public function update_foto()
    {
        ...
        $this->db->trans_start();
        foreach ($input as $value) {
            $nis  = trim($value['nis'] ?? '00');
            $foto = 'uploads/foto_siswa/' . $nis . '.jpg';

            if (isset($value['foto'])) {
                $extension = $value['ext'] === 'jpeg' ? 'jpg' : $value['ext'];
                $output    = $nis . '.' . $extension;
                file_put_contents('./uploads/foto_siswa/' . $output, base64_decode($value['foto']));
                $foto = 'uploads/foto_siswa/' . $output;
            }

            $save = $this->db->update('master_siswa', ['nis' => $nis, 'foto' => $foto], ['id_siswa' => $value['id']]);
        }
        $this->db->trans_complete();
    ...
    public function editLogin()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->output_json(['status' => false, 'text' => validation_errors()]);
            return;
        }
        ...
        $reg = $this->registerSiswa(
            ...
            trim($pass ?? ''),
            $email,
            ['first_name' => $nama[0], 'last_name' => end($nama)],
            ['3']
        );

        $this->output_json($reg);
    }
